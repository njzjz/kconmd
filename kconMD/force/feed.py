...
class Feed(object):
    ...
    def get_feed_dict(self,atoms_or_trajectory,index):
        ...
        indexing = transformed.indexing.reshape((1, 3*(len(self._transformer._species)-self._transformer._num_ghosts), -1))
        ...
        for distance in distances_cutoff:
            ...
            if symbol in atom_num:
                ...
                if symbol in self.maxatoms:
                    atom_num[symbol]=1
                    indexs.append(distance[1])
        traj=Atoms([self.atoms[j] for j in indexs],cell=self.atoms.get_cell())
        traj.wrap(center=self.atoms[i].position/self.atoms.get_cell_lengths_and_angles()[0:3],pbc=self.atoms.get_pbc())
